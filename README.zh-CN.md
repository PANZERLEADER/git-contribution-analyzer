...
GCA 0.5.1 在全量重建索引时保留身份合并，并自动迁移现有工作区；同时包含 0.5.0 引入的
observation-only 历史结构基线和 PySide6/QML 桌面工作台，覆盖仓库生命周期、评估、趋势、
身份、Evidence、历史对比、导出、简历和 Provider 健康检查：
...
GCA 不根据提交数或代码行数推断员工价值、工时、绩效等级、薪酬或晋升建议。用户可显式启用
基于公开规则的多维排名，并通过权重配置生成 cohort 内可审计总分。
...
报告包含 Evidence ID、规则版本、置信度和 gaps。可重复 `--rank-by` 启用 workload、difficulty、
...
- [0.5.1 发布说明](doc/zh-CN/release-0.5.1.md)
